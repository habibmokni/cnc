...
import { CncStore, CncStoreProduct, CncNearbyStore } from '../types/store.type';
...
> {
	...
	public readonly currentLocation = signal<google.maps.LatLngLiteral | null>(null);
	public readonly distanceInKm = signal<number[]>([]);
	public readonly selectedStore = signal<TStore | null>(null);
	public readonly directionsResult = signal<google.maps.DirectionsResult | undefined>(undefined);
	...
	public checkAllProductsAvailability(cart: readonly CncCartItem[]): CncNearbyStore<TStore>[] {
		const distances = this.distanceInKm();
		const results: CncNearbyStore<TStore>[] = [];
		this.stores().forEach((store, i) => {
			let allInStock = true;
	...
	public filterByCartAvailability(cart: readonly CncCartItem[]): Readonly<{
		stores: TStore[];
		locations: google.maps.LatLngLiteral[];
	}> {
		const allStores = this.stores();
